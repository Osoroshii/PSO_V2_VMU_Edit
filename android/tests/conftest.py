...
def _detect_kivy_window_support():
    ...
    (GitHub's Windows-hosted runners hit a different, apparently unrelated
    problem -- pytest hung for the full CI job timeout before even reaching
    its first "collecting ..." line, i.e. before this function, or any test
    file, ever ran. Chased across three attempts here before realizing the
    real fix belonged in the workflow, not this probe: windows-latest is
    excluded from the android-test job's matrix entirely -- see the comment
    in .github/workflows/ci.yml for the fuller story.)"""
